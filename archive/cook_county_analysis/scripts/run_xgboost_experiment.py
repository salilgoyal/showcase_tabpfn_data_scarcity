#!/usr/bin/env python3
"""
XGBoost Data Scarcity Experiment with CBG-matched test sets.

Evaluates XGBoost with hyperparameter tuning on:
1. Full test set (all 2022 data)
2. CBG-matched test set (only 2022 data from CBGs seen in training)

Outputs:
- results/xgboost_full.csv
- results/xgboost_cbg_matched.csv
- /nlp/scr/salilg/cook_county_predictions/xgboost/...
"""

#SBATCH --account=nlp
#SBATCH --job-name=xgb_exp
#SBATCH --cpus-per-task=8
#SBATCH --mem=48G
#SBATCH --gres=gpu:1
#SBATCH --partition=jag-standard
#SBATCH --time=8-0
#SBATCH --output=logs/xgboost_experiment.out
#SBATCH --error=logs/xgboost_experiment.err
...
def save_predictions(y_true, y_pred, X_test, seed, train_size, test_type,
                    predictions_dir, cbg_column, model_name='xgboost'):
    """
    ...
    Args:
        ...
        model_name: Name of model (default: 'xgboost')
    """
    try:
...
def run_single_experiment(model_runner, X_train_pool, y_train_pool,
    ...
    Args:
        model_runner: Instance of XGBoostRunner
    ...
    try:
        ...
        tune_time = model_runner.get_tune_time()
        ...
        # Calculate metrics
        # ====================================================================
        # PASS LOG_TRANSFORMED FLAG TO HANDLE EVELYN'S PREPROCESSING
        # ====================================================================
        ...
        result = create_result_dict(
            ...
            0, 0, {}, 0, 0, tune_time=0, status=f'failed: {str(e)}'
        )
...
def run_full_experiment(experiment_name, data_path, output_dir='results',
                       ...
                       n_iter=10, cv=3, use_gpu=True, test_type='full'):
    """
    Run the full XGBoost data scarcity experiment.

    Args:
        experiment_name: Name for this experiment (required, used in output filenames)
        data_path: Path to cook_county.csv
        output_dir: Directory to save results
        train_sizes: List of training sizes (default: 25 to 1000 by 25)
        ...
        cv: Number of CV folds for hyperparameter tuning (default: 3)
        use_gpu: Use GPU for XGBoost (default: True)
        test_type: 'full' or 'cbg_matched' (default: 'full')
    """
    ...
    logger.info("="*80)
    logger.info(f"XGBOOST DATA SCARCITY EXPERIMENT: {experiment_name}")
    logger.info("="*80)
    ...
    if predictions_dir:
        predictions_dir_full = os.path.join(predictions_dir, experiment_name, 'xgboost')
    ...
    model_runner = XGBoostRunner(n_iter=n_iter, cv=cv, use_gpu=use_gpu)
    ...
    logger.info(f"Hyperparameter search: {n_iter} iterations, {cv}-fold CV")
    ...
    for seed in seeds:
        for train_size in train_sizes:
            ...
            if experiment_num % 20 == 0:
                ...
                output_file = os.path.join(exp_output_dir, 'xgboost.csv')
    ...
    # Summary statistics
    logger.info("\nAverage tuning time by train size:")
    df_results_success = df_results[df_results['status'] == 'success']
    if len(df_results_success) > 0:
        tune_summary = df_results_success.groupby('train_size')['tune_time'].mean()
        logger.info(f"\n{tune_summary}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='XGBoost Data Scarcity Experiment')
    ...
    parser.add_argument('--n_iter', type=int,
                       default=10,
                       help='Number of hyperparameter combinations (default: 10)')
    ...
    if args.config:
        from src.config_loader import load_config, setup_preprocessing_from_config, get_experiment_params, get_paths_config, get_model_config
        ...
        model_config = get_model_config(_CONFIG, 'xgboost')
        ...
        # Get XGBoost hyperparameters from config
        if model_config and 'hyperparameters' in model_config:
            hparams = model_config['hyperparameters']
            args.n_iter = hparams.get('n_iter', args.n_iter)
            args.cv = hparams.get('cv', args.cv)
            args.no_gpu = not hparams.get('use_gpu', True)  # Invert because flag is --no_gpu
    ...
    print(f"Results saved to: {args.output_dir}/{args.experiment_name}/xgboost.csv")
    print(f"Logs saved to: {log_dir}/xgboost.log")
    if predictions_dir:
        print(f"Predictions saved to: {predictions_dir}/{args.experiment_name}/xgboost/")
    print("="*80)
