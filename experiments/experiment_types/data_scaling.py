...
from src.data.preprocessing import load_and_prepare_data as preprocess_data
...
class DataScalingExperiment(BaseExperimentRunner):
    ...
    def load_and_prepare_data(self) -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]:
        """
        Load dataset and apply preprocessing using the centralized preprocessing pipeline.

        This now routes through src/data/preprocessing.py::load_and_prepare_data() to ensure
        consistency with the original Cook County experiments. The preprocessing pipeline handles:
        - Feature selection (property chars, census, assessed value, etc.)
        - Data cleaning (drop nulls, single-value cols, repeat sales, etc.)
        ...
        - Temporal train-test split (2021 train, 2022 test)
        ...
        if 'cook_county_csv' in data_config:
            # Cook County mode
            data_path = str(resolve_data_path(data_config['cook_county_csv']))
            logger.info(f"Loading Cook County data from: {data_path}")

        elif 'county_csv_path' in data_config:
            # Single county mode
            data_path = str(resolve_data_path(data_config['county_csv_path']))
        ...
        # ========================================================================
        # EXTRACT PREPROCESSING CONFIGS
        # ========================================================================
        preprocessing_config = self.config.get('preprocessing')
        
        if not preprocessing_config:
            # No preprocessing - just load raw data and do simple split
            logger.warning("No preprocessing config found - using raw data")
        ...
        # ========================================================================
        # CALL CENTRALIZED PREPROCESSING PIPELINE
        # This is the key change - we now use the tested preprocessing pipeline
        # that was used in the original Cook County experiments, ensuring we get
        # the same preprocessing steps (winsorization, normalization, one-hot
        # encoding, temporal features, etc.)
        # ========================================================================
        feature_config = preprocessing_config.get('features', {})
        step_config = preprocessing_config.get('steps', {})
        cbg_column = data_config.get('cbg_column', 'block_group_id')

        logger.info("="*80)
        logger.info("Using centralized preprocessing pipeline from src/data/preprocessing.py")
        logger.info("="*80)

        # Call the preprocessing function
        X_train_pool, y_train_pool, X_test, y_test, _ = preprocess_data(
            data_path=data_path,
            feature_config=feature_config,
            step_config=step_config,
            cbg_column=cbg_column
        )
