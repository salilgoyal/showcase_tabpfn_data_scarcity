"""
Test TabPFN performance with a small sample of real property tax data.
Memory-efficient version using row_group filtering.
"""
...
# Read just the first 15K rows (memory efficient)
print(f"Loading first 15,000 rows from real data...")
df = pd.read_parquet(DATA_FILE, engine='pyarrow')
df_sample = df.head(15000)
print(f"Loaded: {len(df_sample):,} rows, {df_sample.shape[1]} columns")
...
print("Fitting...")
t0 = time.time()
model1.fit(X_train_processed, y_train.values)
t1 = time.time()
fit_time_1 = t1 - t0
...
print("Fitting...")
t0 = time.time()
model2.fit(X_train_processed, y_train.values)
t1 = time.time()
fit_time_2 = t1 - t0
